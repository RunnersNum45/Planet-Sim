...
def grav_change(val): #Used for the gravtational constant changing slider
	global G
	G = int(val)
	print (G)
	return G
...
widget.pack()
speed = Scale(master, orient="horizontal", from_=1, to_=1000, command=fps_change)
gravity = Scale(master, orient="horizontal", from_=0, to_=1000, command=grav_change)
...
def main(planets):
	for x in range(len(planets)):
		...
		for other in planets[x+1:]:
			F, angle = (max(G, 0.00000000000000001)*current[2]*other[2])/(current.distance(other)**2), current.angle(other)
			current.velo[0], current.velo[1] = (current.velo[0]+math.sin(angle)*(F/current.mass)), (current.velo[1]+math.cos(angle)*(F/current.mass))
			other.velo[0], other.velo[1] = (other.velo[0]+math.sin((angle+3.14159)%6.28318530718)*(F/other.mass)), (other.velo[1]+math.cos((angle+3.14159)%6.28318530718)*(F/other.mass))
...
def off(): return [(planets_list[0].pos[0]-di[0]/2)*CENTER, (planets_list[0].pos[1]-di[1]/2)*CENTER]
